...
public class GameRecommender {
    ...
    public GameRecommender(Reader dataInput) {
        try (var reader = new BufferedReader(dataInput)) {
            this.games = reader.lines().skip(1).map(Game::of).toList();
        } catch (IOException e) {
    ...
    public Game getHighestUserRatedGameByPlatform(String platform) {
        return this.games.stream()
                ...
                .orElseThrow(() -> new NoSuchElementException("No games for this platform are released"));
    }
    ...
    public int getYearsActive(String platform) {
        ...
        Game last = null;
        try {
            first = this.games.stream()
                    .filter(game -> game.platform().equals(platform))
                    .min(Comparator.comparing(Game::releaseDate))
                    .orElseThrow(() -> new NoSuchElementException("No games for this platform are released"));
            last = this.games.stream()
                    .filter(game -> game.platform().equals(platform))
                    .max(Comparator.comparing(Game::releaseDate))
                    .orElseThrow(() -> new NoSuchElementException("No games for this platform are released"));
        } catch (NoSuchElementException e) {
            return 0;
        }

        int firstYear = first.releaseDate().getYear();
        int lastYear = last.releaseDate().getYear();
    ...
    public List<Game> getGamesSimilarTo(String... keywords) {
        Stream<Game> stream = this.games.stream();

        for (String keyword : keywords) {
            stream = stream.filter(game -> game.summary().contains(keyword));
        }
